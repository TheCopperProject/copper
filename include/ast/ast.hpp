...
struct Type {
    ...
    static Type* makePrimitive(ArenaAllocator& arena, TypeKind kind,
                               Span span) {
    ...
    static Type* makeNamed(ArenaAllocator& arena, std::string_view name,
                           Span span) {
        Type* t = arena.create<Type>();
    ...
    static Type* makeArray(ArenaAllocator& arena, Type* element, uint64_t size,
                           Span span) {
        Type* t = arena.create<Type>();
    ...
    static Type* makeFunc(ArenaAllocator& arena, Type** params,
                          uint32_t param_count, Type* return_type, Span span) {
        Type* t = arena.create<Type>();
...
struct Expr {
    ...
    static Expr* makeLiteralInt(ArenaAllocator& arena, int64_t value,
                                Span span) {
    ...
    static Expr* makeLiteralFloat(ArenaAllocator& arena, double value,
                                  Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeLiteralString(ArenaAllocator& arena, const char* value,
                                   uint32_t length, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeLiteralArray(ArenaAllocator& arena, Expr** values,
                                  uint32_t value_count, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeLiteralStruct(ArenaAllocator& arena, Type* type,
                                Field* fields, uint32_t field_count,
                                Span span) {
        Expr* e = arena.create<Expr>();
        e->kind = ExprKind::LITERAL_STRUCT;
        e->span = span;
        e->literal_struct = {type, fields, field_count};
        return e;
    }

    static Expr* makeIdentifier(ArenaAllocator& arena, std::string_view name,
                                bool is_global, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeBinary(ArenaAllocator& arena, TokenKind op, Expr* left,
                            Expr* right, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeUnary(ArenaAllocator& arena, TokenKind op, Expr* operand,
                           Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeAssign(ArenaAllocator& arena, TokenKind op, Expr* target,
    ...
    static Expr* makeCall(ArenaAllocator& arena, Expr* callee, Expr** args,
                          uint32_t arg_count, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeIndex(ArenaAllocator& arena, Expr* array, Expr* index,
                           Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeMember(ArenaAllocator& arena, Expr* object,
                            std::string_view field, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeArrowMember(ArenaAllocator& arena, Expr* object,
                                 std::string_view field, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeCast(ArenaAllocator& arena, Expr* operand,
                          Type* target_type, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makeTernary(ArenaAllocator& arena, Expr* cond, Expr* then_b,
                             Expr* else_b, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makePreIncDec(ArenaAllocator& arena, TokenKind op,
                               Expr* operand, Span span) {
        Expr* e = arena.create<Expr>();
    ...
    static Expr* makePostIncDec(ArenaAllocator& arena, TokenKind op,
                                Expr* operand, Span span) {
        Expr* e = arena.create<Expr>();
...
struct Stmt {
    ...
    static Stmt* makeBlock(ArenaAllocator& arena, Stmt** stmts, uint32_t count,
                           Span span) {
        Stmt* s = arena.create<Stmt>();
    ...
    static Stmt* makeVarDecl(ArenaAllocator& arena, std::string_view name,
                             Type* type, Expr* init, bool is_mutable,
                             bool is_global, Span span) {
        Stmt* s = arena.create<Stmt>();
    ...
    static Stmt* makeFuncDecl(ArenaAllocator& arena, std::string_view name,
                              Field* params, uint32_t param_count,
                              Type* return_type, Stmt* body, bool is_extern,
                              bool is_variadic, bool hasExplicitReturnType,
                              Span span) {
        Stmt* s = arena.create<Stmt>();
        s->kind = StmtKind::FUNC_DECL;
        s->span = span;
        s->func_decl = {name, params,      param_count, return_type,
                        body, is_variadic, is_extern,   hasExplicitReturnType};
        return s;
    }

    static Stmt* makeStructDecl(ArenaAllocator& arena, std::string_view name,
                                Field* fields, uint32_t field_count,
                                Span span) {
        Stmt* s = arena.create<Stmt>();
    ...
    static Stmt* makeUnionDecl(ArenaAllocator& arena, std::string_view name,
                               Field* fields, uint32_t field_count, Span span) {
        Stmt* s = arena.create<Stmt>();
    ...
    static Stmt* makeIf(ArenaAllocator& arena, Expr* cond, Stmt* then_b,
                        Stmt* else_b, Span span) {
        Stmt* s = arena.create<Stmt>();
    ...
    static Stmt* makeWhile(ArenaAllocator& arena, Expr* cond, Stmt* body,
                           Span span) {
        Stmt* s = arena.create<Stmt>();
    ...
    static Stmt* makeDoWhile(ArenaAllocator& arena, Expr* cond, Stmt* body,
                             Span span) {
        Stmt* s = arena.create<Stmt>();
    ...
    static Stmt* makeFor(ArenaAllocator& arena, Stmt* init, Expr* cond,
                         Expr* step, Stmt* body, Span span) {
        Stmt* s = arena.create<Stmt>();
    ...
    static Stmt* makeSwitch(ArenaAllocator& arena, Expr* subject,
                            CaseClause* cases, uint32_t case_count,
                            bool is_match, Span span) {
        Stmt* s = arena.create<Stmt>();
...
struct SourceFile {
    uint32_t file_id = 0;
    PackageDecl* package = nullptr;
    ImportDecl** imports = nullptr;
