...
void AstPrinter::printExpr(Expr* e, int depth, std::string& out) {
    ...
    switch (e->kind) {
        ...
        case ExprKind::LITERAL_CHAR:
            out +=
                std::string("CharLiteral('") + e->literal_char.value + "')\n";
            break;
        ...
        case ExprKind::LITERAL_BOOL:
            out += std::string("BoolLiteral(") +
                   (e->literal_bool.value ? "true" : "false") + ")\n";
            break;
        ...
        case ExprKind::LITERAL_STRUCT:
            out += std::string("StructLiteral(");
            printType(e->literal_struct.target, out);
            out += std::string(") {\n");

            for (size_t i = 0; i < e->literal_struct.field_count; i++) {
                auto field = e->literal_struct.fields[i];
                indent(depth + 1, out);
                out += std::format("{}: ", field.name);
                out += std::string("{\n");
                printExpr(field.default_value, depth + 2, out);
                indent(depth + 1, out);
                out += std::string("}\n");
            }
        ...
        case ExprKind::IDENTIFIER:
            out += std::string("Identifier(" + std::string(e->identifier.name) +
                               ", ");
            out += std::string((e->identifier.is_global ? "true" : "false"));
            out += std::string(")\n");
            break;
        ...
        case ExprKind::POST_INC_DEC:
            out += std::string("PostIncDec(") +
                   tokenOpName(e->post_inc_dec.op) + ")\n";
...
void AstPrinter::printStmt(Stmt* s, int depth, std::string& out) {
    ...
    switch (s->kind) {
        ...
        case StmtKind::VAR_DECL:
            out += std::string(s->kind == StmtKind::CONST_DECL ? "LetDecl("
                                                               : "VarDecl(") +
                   std::string(s->var_decl.name) + ": ";
        ...
        case StmtKind::FUNC_DECL:
            ...
            for (uint32_t i = 0; i < s->func_decl.param_count; i++) {
                indent(depth + 1, out);
                out +=
                    "param " + std::string(s->func_decl.params[i].name) + ": ";
        ...
        case StmtKind::STRUCT_DECL:
            out +=
                "StructDecl(" + std::string(s->struct_union_decl.name) + ")\n";
        ...
        case StmtKind::UNION_DECL:
            out +=
                "UnionDecl(" + std::string(s->struct_union_decl.name) + ")\n";
            for (uint32_t i = 0; i < s->struct_union_decl.field_count; i++) {
                indent(depth + 1, out);
                out += std::string(s->struct_union_decl.fields[i].name) + ": ";
                printType(s->struct_union_decl.fields[i].type, out);
...
std::string AstPrinter::print(SourceFile* file) {
    ...
    if (file->package && file->package->path_count > 0) {
        out += "  Package: ";
        for (uint32_t i = 0; i < file->package->path_count; i++) {
            out += std::string(file->package->path[i]);
            if (i + 1 < file->package->path_count) {
                out += ".";
            }
        }
        out += "\n";
    ...
    if (file->imports && file->import_count > 0) {
        out += "  Imports:\n";
        for (uint32_t i = 0; i < file->import_count; i++) {
            ImportDecl* imp = file->imports[i];
            if (!imp)
                continue;

            out += "    import ";
            for (uint32_t j = 0; j < imp->path_count; j++) {
                out += std::string(imp->path[j]);
                if (j + 1 < imp->path_count) {
                    out += ".";
                }
            }
            out += ";\n";
