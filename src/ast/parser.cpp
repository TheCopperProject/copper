...
SourceFile* Parser::parseSourceFile() {
    ...
    PackageDecl* packageDecl = nullptr;
    ...
    while (!isAtEnd()) {
        ...
        if (check(TokenKind::PACKAGE_KEYWORD)) {
            if (packageDecl != nullptr) {
                errors_.error("multiple package declarations are not allowed", current().span);
            }
            packageDecl = parsePackageDecl();
        } else if (check(TokenKind::IMPORT_KEYWORD)) {
    ...
    file->package = packageDecl;

    file->import_count = static_cast<uint32_t>(imports.size());
    file->imports = arena_.create_array<ImportDecl*>(imports.size());
    for (std::size_t i = 0; i < imports.size(); i++)
        file->imports[i] = imports[i];
...
PackageDecl* Parser::parsePackageDecl() {
    ...
    do {
        if (!check(TokenKind::IDENTIFIER_TOKEN)) {
            errors_.error("expected identifier in package path", current().span);
            break;
        }
        path.push_back(advance().source);
    } while (match(TokenKind::DOT_TOKEN));

    expect(TokenKind::SEMICOLON_TOKEN, "';' after package declaration");

    PackageDecl* pkg = arena_.create<PackageDecl>();
    pkg->path_count = static_cast<uint32_t>(path.size());
    pkg->path = arena_.create_array<std::string_view>(path.size());
    for (std::size_t i = 0; i < path.size(); ++i) {
        pkg->path[i] = path[i];
    }
    pkg->span = spanFrom(start);
    return pkg;
}
...
ImportDecl* Parser::parseImportDecl() {
    ...
    do {
        if (!check(TokenKind::IDENTIFIER_TOKEN)) {
            errors_.error("expected identifier in import path", current().span);
            break;
        }
        path.push_back(advance().source);
    } while (match(TokenKind::DOT_TOKEN));

    expect(TokenKind::SEMICOLON_TOKEN, "';' after import declaration");

    ImportDecl* imp = arena_.create<ImportDecl>();
    imp->path_count = static_cast<uint32_t>(path.size());
    imp->path = arena_.create_array<std::string_view>(path.size());
    for (std::size_t i = 0; i < path.size(); ++i) {
        imp->path[i] = path[i];
    }
    imp->span = spanFrom(start);
    return imp;
}
...
Stmt* Parser::parseTopLevelDecl() {
    ...
    errors_.error(
        "expected a high level declaration (func, struct, enum, union, "
        "let, var)",
        current().span);
    return nullptr;
}
        ...
        do {
            ...
            if (check(TokenKind::ELLIPSIS_TOKEN)) {
                ...
                *isvariadic = true;
                ...
                if (!check(TokenKind::RIGHT_PAREN_TOKEN)) {
                    errors_.error("no parameters allowed after variadic '...'",
                                  current().span);
                    while (!check(TokenKind::RIGHT_PAREN_TOKEN) &&
                           !check(TokenKind::LEFT_BRACE_TOKEN) && !isAtEnd()) {
                        advance();
                    }
                }
                break;
            ...
            if (!expect(TokenKind::COLON_TOKEN,
...
Stmt* Parser::parseFuncDecl(bool is_extern) {
    ...
    return Stmt::makeFuncDecl(arena_, name, params, paramCount, returnType,
                              body, is_extern, isvariadic,
                              returnType != nullptr, spanFrom(start));
}
...
Stmt* Parser::parseStructDecl() {
    ...
    return Stmt::makeStructDecl(arena_, name, fields, fieldCount,
                                spanFrom(start));
}
...
Stmt* Parser::parseUnionDecl() {
    ...
    return Stmt::makeUnionDecl(arena_, name, fields, fieldCount,
                               spanFrom(start));
}
...
Stmt* Parser::parseEnumDecl() {
    ...
    return Stmt::makeEnumDecl(arena_, name, namesArr, valuesArr, count,
                              spanFrom(start));
}
...
Stmt* Parser::parseBlock() {
    ...
    return Stmt::makeBlock(arena_, arr, static_cast<uint32_t>(stmts.size()),
                           spanFrom(start));
}
...
Stmt* Parser::parseVarDecl(bool is_mutable) {
    ...
    return Stmt::makeVarDecl(arena_, name, type, init, is_mutable, false,
                             spanFrom(start));
}
...
Stmt* Parser::parseSwitchOrMatch(bool is_match) {
    ...
    while (!check(TokenKind::RIGHT_BRACE_TOKEN) && !isAtEnd()) {
        ...
        while (!check(TokenKind::CASE_KEYWORD) &&
               !check(TokenKind::DEFAULT_KEYWORD) &&
    ...
    return Stmt::makeSwitch(arena_, subject, arr,
                            static_cast<uint32_t>(cases.size()), is_match,
                            spanFrom(start));
}
...
Type* Parser::parseType() {
    ...
    if (isTypeKeyword(current().kind)) {
        base = Type::makePrimitive(arena_, primMap.at(current().kind),
                                   current().span);
        advance();
        ...
        base = Type::makeFunc(arena_, arr, static_cast<uint32_t>(params.size()),
                              ret, spanFrom(start));
    } else {
...
Expr* Parser::parseTernary() {
    ...
    if (match(TokenKind::QUESTION_TOKEN)) {
        ...
        return Expr::makeTernary(arena_, cond, thenExpr, elseExpr,
                                 spanFrom(start));
    }
    return cond;
}
...
BIN_LEVEL(parseEquality, parseRelational,
          check(TokenKind::EQUAL_EQUAL_TOKEN) ||
              check(TokenKind::NOT_EQUAL_TOKEN))
BIN_LEVEL(parseRelational, parseShift,
          check(TokenKind::LESS_TOKEN) || check(TokenKind::GREATER_TOKEN) ||
              check(TokenKind::LESS_EQUAL_TOKEN) ||
              check(TokenKind::GREATER_EQUAL_TOKEN))
BIN_LEVEL(parseShift, parseAdditive,
          check(TokenKind::LEFT_SHIFT_TOKEN) ||
              check(TokenKind::RIGHT_SHIFT_TOKEN))
...
BIN_LEVEL(parseMultiplicative, parseUnary,
          check(TokenKind::MULTIPLY_TOKEN) || check(TokenKind::DIVIDE_TOKEN) ||
              check(TokenKind::MODULO_TOKEN))
...
Expr* Parser::parseUnary() {
    ...
    if (check(TokenKind::INCREMENT_TOKEN) ||
        check(TokenKind::DECREMENT_TOKEN)) {
        TokenKind op = advance().kind;
        Expr* operand = parseUnary();
...
Expr* Parser::parsePostfix() {
    ...
    for (;;) {
        if (check(TokenKind::LEFT_PAREN_TOKEN)) {
            ...
            advance();
            if (!check(TokenKind::IDENTIFIER_TOKEN)) {
                errors_.error("expected field's name after '.'",
                              current().span);
                break;
            }
        ...
        } else if (check(TokenKind::INCREMENT_TOKEN) ||
                   check(TokenKind::DECREMENT_TOKEN)) {
            TokenKind op = advance().kind;
            e = Expr::makePostIncDec(arena_, op, e, spanFrom(start));
        } else if (check(TokenKind::AS_KEYWORD)) {
            advance();
            Type* t = parseType();

            e = Expr::makeCast(arena_, e, t, spanFrom(start));
        } else if (check(TokenKind::LEFT_BRACE_TOKEN)) {
            advance();
            if (e->kind != ExprKind::IDENTIFIER) {
                errors_.error(
                    "expected type for a struct literal: StructName {....}",
                    current().span);
            }

            auto type = Type::makeNamed(arena_, e->identifier.name, e->span);
            std::vector<Field> tmp;
            if (!check(TokenKind::RIGHT_BRACE_TOKEN)) {
                do {
                    if (!check(TokenKind::IDENTIFIER_TOKEN)) {
                        errors_.error("expected fields's name", current().span);
                        return nullptr;
                    }
                    auto s1 = peek().span;
                    auto name = advance().source;

                    expect(TokenKind::COLON_TOKEN,
                           "expected ; after field's name");
                    ...
                    field.span = Span::merge(s1, expr->span);

                    tmp.push_back(field);
                } while (match(TokenKind::COMMA_TOKEN));
            }
            expect(TokenKind::RIGHT_BRACE_TOKEN, "'}' after arguments");

            Field* fields = arena_.create_array<Field>(tmp.size());
            for (std::size_t i = 0; i < tmp.size(); i++)
                fields[i] = tmp[i];

            e = Expr::makeLiteralStruct(arena_, type, fields,
                                        static_cast<uint32_t>(tmp.size()),
                                        spanFrom(start));
            break;
        } else {
...
Expr* Parser::parsePrimary() {
    ...
    switch (current().kind) {
        ...
        case TokenKind::STRING_TOKEN: {
            ...
            return Expr::makeLiteralString(
                arena_, v, static_cast<uint32_t>(std::strlen(v)),
                spanFrom(start));
        }
        case TokenKind::IDENTIFIER_TOKEN: {
            ...
            if (name == "true" || name == "false") {
                return Expr::makeLiteralBool(arena_, name == "true",
                                             spanFrom(start));
            }
        ...
        case TokenKind::LEFT_BRACKET_TOKEN: {
            ...
            return Expr::makeLiteralArray(arena_, values,
                                          static_cast<uint32_t>(tmp.size()),
                                          spanFrom(start));
        }
