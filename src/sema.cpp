...
Type *Sema::inferReturnType(Stmt *body, bool &hasReturn)
{
    ...
    {
        error("function has both 'return' with a value and 'return' without a value", body->span);
        return nullptr;
    }
    ...
    for (size_t i = 1; i < returnTypes.size(); ++i)
    {
        ...
        if (isNumericType(common) && isNumericType(t))
        {
            ...
            if (!ok)
            {
                error("incompatible return types: '" + typeToString(common) + "' and '" + typeToString(t) + "'", body->span);
                return nullptr;
            }
        }
        else
...
void Sema::collectReturnTypes(Stmt *s, std::vector<Type *> &types, bool &hasVoid)
{
    if (!s)
        return;
...
{
    return t && (t->kind == TypeKind::FLOAT_CONSTANT || t->kind == TypeKind::F32 || t->kind == TypeKind::F64);
}
...
{
    return isNumericType(t) || (t && (t->kind == TypeKind::CHAR || t->kind == TypeKind::BOOL));
}
...
std::string Sema::typeToString(Type *t)
{
    ...
    case TypeKind::VARIADIC:
        return typeToString(t->wrapper.inner) + "[...]";
    case TypeKind::ARRAY:
        return typeToString(t->array.element) + "[" + std::to_string(t->array.size) + "]";
    case TypeKind::SLICE:
...
bool Sema::typesEqual(Type *a, Type *b)
{
    ...
    case TypeKind::ARRAY:
        return a->array.size == b->array.size && typesEqual(a->array.element, b->array.element);
    case TypeKind::SLICE:
...
bool Sema::isAssignable(Type *from, Type *to)
{
    ...
    if (to->kind == TypeKind::OPTIONAL)
        return isAssignable(from, to->wrapper.inner) ||
               (from->kind == TypeKind::OPTIONAL && isAssignable(from->wrapper.inner, to->wrapper.inner));

    if (from->kind == TypeKind::ARRAY && to->kind == TypeKind::ARRAY)
        return from->array.size == to->array.size && isAssignable(from->array.element, to->array.element);
...
bool Sema::checkAssignableTarget(Expr *target)
{
    if (!isLValue(target))
    {
        error("left-hand side of assignment must be a variable, index, member, or dereference expression", target->span);
        return false;
    }
    ...
    if (target->kind == ExprKind::IDENTIFIER)
    {
        ...
        if (sym)
        {
            ...
            {
                error("cannot assign to function '" + std::string(target->identifier.name) + "'", target->span);
                return false;
            }
            if (!sym->is_mutable)
            {
                error("cannot assign to immutable variable '" + std::string(target->identifier.name) +
...
void Sema::collectTypes(SourceFile *file)
{
    for (uint32_t i = 0; i < file->decl_count; i++)
    {
        ...
        if (d->kind == StmtKind::STRUCT_DECL || d->kind == StmtKind::UNION_DECL || d->kind == StmtKind::ENUM_DECL)
        {
...
void Sema::validateTypeBodies(SourceFile *file)
{
    for (uint32_t i = 0; i < file->decl_count; i++)
    {
        ...
        if (d->kind == StmtKind::STRUCT_DECL || d->kind == StmtKind::UNION_DECL)
        {
            std::string ownerName(d->struct_union_decl.name);
            const char *kindWord = d->kind == StmtKind::STRUCT_DECL ? "struct" : "union";
            ...
            for (uint32_t f = 0; f < d->struct_union_decl.field_count; f++)
            {
                ...
                if (seen.count(fieldName))
                    error("duplicate field '" + fieldName + "' in " + kindWord + " '" + ownerName + "'", field.span);
                seen.insert(fieldName);

                resolveType(field.type);

                if (field.type && field.type->kind == TypeKind::NAMED && field.type->named.name == ownerName)
                    error(kindWord + std::string(" '") + ownerName + "' contains itself as field '" + fieldName +
                              "' without indirection; use a pointer, optional, or slice instead",
                          field.span);

                if (field.default_value)
                {
                    Type *defType = checkExpr(field.default_value);
                    if (field.type && defType && !isAssignable(defType, field.type))
                        error("default value for field '" + fieldName + "' has type '" + typeToString(defType) +
                                  "' but expected '" + typeToString(field.type) + "'",
        ...
        else if (d->kind == StmtKind::ENUM_DECL)
        {
            ...
            for (uint32_t v = 0; v < d->enum_decl.variant_count; v++)
            {
                ...
                if (seen.count(variantName))
                    error("duplicate enum variant '" + variantName + "' in enum '" + ownerName + "'", d->span);
                ...
                if (d->enum_decl.variant_values[v])
                {
                    ...
                    if (valType && !isIntegerType(valType))
                        error("enum variant '" + variantName + "' must have an integer value, got '" +
...
void Sema::collectSignatures(SourceFile *file)
{
    for (uint32_t i = 0; i < file->decl_count; i++)
    {
        ...
        if (d->kind == StmtKind::FUNC_DECL)
        {
            ...
            for (uint32_t p = 0; p < d->func_decl.param_count; p++)
            {
                ...
                if (paramNames.count(pname))
                    error("duplicate parameter name '" + pname + "' in function '" + name + "'", param.span);
                paramNames.insert(pname);
                resolveType(param.type);
            }

            Type *retType = d->func_decl.return_type ? resolveType(d->func_decl.return_type) : nullptr;
        ...
        else if (d->kind == StmtKind::CONST_DECL || d->kind == StmtKind::VAR_DECL)
        {
            ...
            Type *declaredType = d->var_decl.type ? resolveType(d->var_decl.type) : nullptr;
            Type *initType = d->var_decl.init ? checkExpr(d->var_decl.init) : nullptr;
            ...
            if (!finalType)
            {
                ...
                {
                    error("cannot infer type of '" + name + "' without a type annotation or an initializer", d->span);
                }
            }
...
void Sema::checkGlobalVarInit(Stmt *d)
{
    ...
    if (sym->type)
    {
        if (initType && !isAssignable(initType, sym->type))
            error("cannot initialize '" + name + "' of type '" + typeToString(sym->type) +
...
void Sema::checkFuncBody(Stmt *d)
{
    ...
    if (!d->func_decl.has_explicit_return_type && retType == nullptr)
    {
        ...
        Type *inferred = inferReturnType(d->func_decl.body, hasReturn);
        if (inferred)
        {
    ...
    if (retType && retType->kind != TypeKind::VOID && !stmtAlwaysReturns(d->func_decl.body))
    {
        error("function '" + name + "' does not return a value of type '" + typeToString(retType) +
                  "' on all code paths",
              d->span);
    }
...
bool Sema::stmtAlwaysReturns(Stmt *s)
{
    ...
    case StmtKind::WHILE_STMT:
    {
        ...
        return cond && cond->kind == ExprKind::LITERAL_BOOL && cond->literal_bool.value;
    }
    ...
    case StmtKind::MATCH_STMT:
    {
        ...
        return hasDefault && allCasesReturn && s->switch_stmt.case_count > 0;
    }
...
void Sema::checkStmt(Stmt *s)
{
    ...
    case StmtKind::BREAK_STMT:
        if (loop_depth_ == 0 && switch_depth_ == 0)
            error("'break' statement is not allowed outside of a loop or switch", s->span);
        break;

    case StmtKind::CONTINUE_STMT:
        if (loop_depth_ == 0)
            error("'continue' statement is not allowed outside of a loop", s->span);
        break;
...
void Sema::checkVarDecl(Stmt *s)
{
    ...
    Type *declaredType = s->var_decl.type ? resolveType(s->var_decl.type) : nullptr;
    ...
    if (!finalType)
    {
        ...
        {
            error("cannot infer type of '" + name + "' without a type annotation or an initializer", s->span);
        }
    }
    else if (initType && !isAssignable(initType, finalType))
    {
        error("cannot initialize '" + name + "' of type '" + typeToString(finalType) +
                  "' with a value of type '" + typeToString(initType) + "'",
...
void Sema::checkIf(Stmt *s)
{
    ...
    if (condType && !isBoolType(condType))
        error("condition of 'if' statement must be of type 'bool', got '" + typeToString(condType) + "'",
              s->if_stmt.cond->span);
...
void Sema::checkWhile(Stmt *s)
{
    ...
    if (condType && !isBoolType(condType))
        error("condition of 'while' statement must be of type 'bool', got '" + typeToString(condType) + "'",
...
void Sema::checkDoWhile(Stmt *s)
{
    ...
    Type *condType = checkExpr(s->while_stmt.cond);
    if (condType && !isBoolType(condType))
        error("condition of 'do-while' statement must be of type 'bool', got '" + typeToString(condType) + "'",
              s->while_stmt.cond->span);
}
...
void Sema::checkFor(Stmt *s)
{
    ...
    if (s->for_stmt.cond)
    {
        ...
        if (condType && !isBoolType(condType))
            error("condition of 'for' statement must be of type 'bool', got '" + typeToString(condType) + "'",
...
void Sema::checkSwitch(Stmt *s)
{
    ...
    for (uint32_t i = 0; i < s->switch_stmt.case_count; i++)
    {
        ...
        {
            if (sawDefault)
                error("multiple 'default' cases in the same switch statement", cc.span);
            sawDefault = true;
        }
        else
        {
            Type *caseType = checkExpr(cc.value);
            if (subjectType && caseType && !isAssignable(caseType, subjectType) && !isAssignable(subjectType, caseType))
                error("case value of type '" + typeToString(caseType) +
                          "' does not match the switch subject type '" + typeToString(subjectType) + "'",
                      cc.value->span);
        }
...
void Sema::checkReturn(Stmt *s)
{
    ...
    {
        error("'return' statement is not allowed outside of a function", s->span);
        if (s->return_stmt.value)
            checkExpr(s->return_stmt.value);
        return;
    }

    Type *valType = s->return_stmt.value ? checkExpr(s->return_stmt.value) : nullptr;

    bool isVoid = (current_return_type_ == nullptr || current_return_type_->kind == TypeKind::VOID);

    if (isVoid)
    {
        if (s->return_stmt.value)
            error("function '" + current_function_name_ + "' has no return value, but a value of type '" +
    ...
    {
        if (!s->return_stmt.value)
            error("function '" + current_function_name_ + "' expects a return value of type '" +
                      typeToString(current_return_type_) + "', but none was provided",
                  s->span);
        else if (valType && !isAssignable(valType, current_return_type_))
            error("cannot return a value of type '" + typeToString(valType) + "' from function '" +
                      current_function_name_ + "' expecting '" + typeToString(current_return_type_) + "'",
                  s->return_stmt.value->span);
    }
}
...
Type *Sema::checkExpr(Expr *e)
{
    ...
    case ExprKind::LITERAL_STRING:
        e->resolved_type = Type::makePointer(arena_, getPrimitive(TypeKind::CHAR), e->span);
        return e->resolved_type;
    ...
    case ExprKind::IDENTIFIER:
    {
        ...
        if (!sym)
        {
            if (types_.count(name))
                ...
                error("use of undeclared identifier '" + name + "'", e->span);
            return nullptr;
        }
    ...
    case ExprKind::PRE_INC_DEC:
        return checkIncDec(e->pre_inc_dec.op, e->pre_inc_dec.operand, e->span);

    case ExprKind::POST_INC_DEC:
        return checkIncDec(e->post_inc_dec.op, e->post_inc_dec.operand, e->span);
    ...
    for (uint32_t i = 1; i < e->literal_array.value_count; i++)
    {
        ...
        if (elemType && t && !typesEqual(elemType, t))
        {
            ...
            if (isNumericType(elemType) && isNumericType(t) && ok)
                ...
                error("array literal has mismatched element types: '" + typeToString(elemType) +
                          "' and '" + typeToString(t) + "'",
                      e->literal_array.values[i]->span);
        }
    }

    Type *arrType = Type::makeArray(arena_, elemType, e->literal_array.value_count, e->span);
...
Type *Sema::checkBinary(Expr *e)
{
    ...
    {
        if (!isBoolType(lt))
            error("left operand of '" + opName(op) + "' must be of type 'bool', got '" + typeToString(lt) + "'",
                  e->binary.left->span);
        if (!isBoolType(rt))
            error("right operand of '" + opName(op) + "' must be of type 'bool', got '" + typeToString(rt) + "'",
                  e->binary.right->span);
        e->resolved_type = getPrimitive(TypeKind::BOOL);
        return e->resolved_type;
    }
    ...
    case TokenKind::NOT_EQUAL_TOKEN:
    {
        bool comparable = typesEqual(lt, rt) ||
                          ...
                          (lt->kind == TypeKind::POINTER && rt->kind == TypeKind::POINTER);
        if (!comparable)
            error("cannot compare values of type '" + typeToString(lt) + "' and '" + typeToString(rt) + "'",
                  e->span);
        e->resolved_type = getPrimitive(TypeKind::BOOL);
        return e->resolved_type;
    }
    ...
    case TokenKind::GREATER_EQUAL_TOKEN:
    {
        ...
        {
            error("relational operator '" + opName(op) + "' requires numeric operands, got '" +
                      typeToString(lt) + "' and '" + typeToString(rt) + "'",
                  e->span);
            return nullptr;
        }
        bool ok;
        promoteNumeric(lt, rt, ok);
        if (!ok)
            error("cannot compare '" + typeToString(lt) + "' with '" + typeToString(rt) +
    ...
    case TokenKind::RIGHT_SHIFT_TOKEN:
    {
        ...
        {
            error("bitwise operator '" + opName(op) + "' requires integer operands, got '" +
                      typeToString(lt) + "' and '" + typeToString(rt) + "'",
                  e->span);
            return nullptr;
        }
        ...
        if (!ok)
        {
            error("operands of '" + opName(op) + "' have mismatched signedness: '" + typeToString(lt) +
                      "' and '" + typeToString(rt) + "'",
                  e->span);
            return nullptr;
        }
    ...
    default:
    {
        if (!isNumericType(lt) || !isNumericType(rt))
        {
            error("invalid operands to binary '" + opName(op) + "': '" + typeToString(lt) + "' and '" +
                      typeToString(rt) + "'",
                  e->span);
            return nullptr;
        }

        if (op == TokenKind::MODULO_TOKEN && (isFloatType(lt) || isFloatType(rt)))
        {
            error("operator '%' requires integer operands, got '" + typeToString(lt) + "' and '" +
                      typeToString(rt) + "'",
                  e->span);
            return nullptr;
        }
...
Type *Sema::checkUnary(Expr *e)
{
    ...
    if (op == TokenKind::AMPERSAND_TOKEN)
    {
        ...
        if (!isLValue(operand))
        {
            error("cannot take the address of a non-lvalue expression", e->span);
            return nullptr;
        }
    ...
    if (op == TokenKind::MULTIPLY_TOKEN)
    {
        ...
        {
            error("cannot dereference a value of non-pointer type '" + typeToString(t) + "'", e->span);
            return nullptr;
        }
    ...
    case TokenKind::LOGICAL_NOT_TOKEN:
        if (!isBoolType(t))
            error("operand of '!' must be of type 'bool', got '" + typeToString(t) + "'", e->span);
        e->resolved_type = getPrimitive(TypeKind::BOOL);
        return e->resolved_type;

    case TokenKind::TILDE_TOKEN:
        if (!isIntegerType(t))
            error("operand of '~' must be an integer type, got '" + typeToString(t) + "'", e->span);
        e->resolved_type = t;
        return t;
    ...
    case TokenKind::PLUS_TOKEN:
        if (!isNumericType(t))
            error("operand of unary '" + opName(op) + "' must be numeric, got '" + typeToString(t) + "'", e->span);
        e->resolved_type = t;
        return t;
...
Type *Sema::checkAssign(Expr *e)
{
    ...
    if (op == TokenKind::ASSIGN_TOKEN)
    {
        if (!isAssignable(valueType, targetType))
            error("cannot assign a value of type '" + typeToString(valueType) + "' to a variable of type '" +
                      typeToString(targetType) + "'",
                  e->assign.value->span);
    }
    else
    {
        bool isBitwiseAssign = op == TokenKind::AND_ASSIGN_TOKEN || op == TokenKind::OR_ASSIGN_TOKEN ||
                               op == TokenKind::XOR_ASSIGN_TOKEN || op == TokenKind::LEFT_SHIFT_ASSIGN_TOKEN ||
                               op == TokenKind::RIGHT_SHIFT_ASSIGN_TOKEN;

        if (isBitwiseAssign)
        {
            if (!isIntegerType(targetType) || !isIntegerType(valueType))
                error("compound assignment '" + opName(op) + "' requires integer operands, got '" +
                          typeToString(targetType) + "' and '" + typeToString(valueType) + "'",
                      e->span);
        }
        else
        {
            ...
            {
                error("compound assignment '" + opName(op) + "' requires numeric operands, got '" +
                          typeToString(targetType) + "' and '" + typeToString(valueType) + "'",
                      e->span);
            }
            else if (!isAssignable(valueType, targetType))
            {
                error("cannot apply '" + opName(op) + "' to a variable of type '" + typeToString(targetType) +
...
Type *Sema::checkCall(Expr *e)
{
    ...
    {
        error("function '" + name + "' expects " + std::to_string(requiredCount) +
                  (isVariadic ? " or more argument(s)" : " argument(s)") + " but " + std::to_string(got) +
                  " were provided",
              e->span);
    }
    ...
    for (uint32_t i = 0; i < checkCount; i++)
    {
        ...
        if (argType && paramType && !isAssignable(argType, paramType))
            error("argument " + std::to_string(i + 1) + " to '" + name + "' has type '" + typeToString(argType) +
                      "' but '" + typeToString(paramType) + "' was expected",
                  e->call.args[i]->span);
    }
...
Type *Sema::checkIndex(Expr *e)
{
    ...
    if (idx && !isIntegerType(idx))
        error("array index must be an integer type, got '" + typeToString(idx) + "'", e->index.index->span);
    ...
    case TypeKind::POINTER:
        ...
        return e->resolved_type;
    default:
        error("cannot index into a value of type '" + typeToString(base) + "'", e->index.array->span);
        return nullptr;
    }
}
...
Type *Sema::checkMember(Expr *e)
{
    ...
    {
        error("cannot access member '" + std::string(e->member.field) + "' on non-struct type '" +
                  typeToString(base) + "'",
              e->span);
        return nullptr;
    }
    ...
    Stmt *decl = it->second;
    if (decl->kind != StmtKind::STRUCT_DECL && decl->kind != StmtKind::UNION_DECL)
    {
        error("type '" + typeName + "' is not a struct or union and has no members", e->span);
        return nullptr;
    }
    ...
    error("'" + typeName + "' has no member named '" + std::string(e->member.field) + "'", e->span);
    return nullptr;
}
...
Type *Sema::checkCast(Expr *e)
{
    ...
    if (!isValidCast(from, to))
        error("invalid cast from '" + typeToString(from) + "' to '" + typeToString(to) + "'", e->span);
...
Type *Sema::checkTernary(Expr *e)
{
    ...
    if (condType && !isBoolType(condType))
        error("condition of ternary expression must be of type 'bool', got '" + typeToString(condType) + "'",
              e->ternary.cond->span);
    ...
    error("branches of ternary expression have incompatible types: '" + typeToString(thenType) + "' and '" +
...
Type *Sema::checkIncDec(TokenKind op, Expr *operand, Span span)
{
    ...
    if (!isNumericType(t))
        error("operand of '" + opName(op) + "' must be numeric, got '" + typeToString(t) + "'", span);

    if (!isLValue(operand))
        error("operand of '" + opName(op) + "' must be an assignable expression", span);
